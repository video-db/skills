# Search & Indexing Guide (v1)

> **LEGACY — do not use for new code.** This page documents the v1 indexing and search API. Every method here **still works in SDK 0.5.0 and is not deprecated** — nothing raises `DeprecationWarning`. The server may attach migration notices to `response.warnings`, surfaced as Python `UserWarning`. Read this only when maintaining existing v1 code.
>
> New code: [../indexing.md](../indexing.md) → [../search.md](../search.md). To port existing code: [../migration.md](../migration.md).
>
> **Routing warning.** In SDK 0.5.0 `video.search()` and `collection.search()` default to **v2**. The calls on this page reach the legacy engine only when you pass a legacy keyword (`search_type=`, `index_type=`, `scene_index_id=`, `result_threshold=`, `namespace=`, and similar) or a positional argument. **`score_threshold=` and `filter=` alone do NOT route to legacy** — they are forwarded to v2. When you mean the legacy engine, call `video.legacy_search(...)` explicitly.
...
- Passing `index_type=` or `scene_index_id=` is also what forces `search()` to route to the legacy engine. Without one of them the call goes to v2.
...
- **Handle "No results found"**: legacy search raises `InvalidRequestError` when no results match. Always wrap legacy search calls in try/except and treat `"No results found"` as an empty result set. v2 search returns an empty response instead of raising.
- **Filter scene search noise**: Semantic scene search can return low-relevance results for vague queries. Use `score_threshold=0.3` (or higher) to filter noise — but only alongside a legacy keyword such as `index_type=`. On its own, `score_threshold=` does not route to legacy, so `video.search(query, score_threshold=0.3)` runs against v2 indexes. Use `video.legacy_search(query, score_threshold=0.3)` when you mean this page's engine.
